...
def compute_error_verts_mod(pred_verts, target_verts=None, target_pose=None, target_shape=None):
    """
    Computes MPJPE over 6890 surface vertices.
    Args:
        verts_gt (Nx6890x3).
    ...
    if target_verts is None:
        ...
        betas = torch.from_numpy(target_shape).to(device)
        pose = torch.from_numpy(target_pose).to(device)
