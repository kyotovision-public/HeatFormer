...
class Renderer:
    ...
    def __init__(self, cfg: CfgNode, faces: np.array):
        ...
        self.img_res = cfg.DATASET.AD_IMG_SIZE

        self.camera_center = [self.img_res // 2, self.img_res // 2]
        self.faces = faces.numpy()
    ...
    def __call__(self,
        ...
        # else:
        #     image = rearrange(torch.from_numpy(image[:, :, ::-1] / 255).float(), 'h w c -> c h w')
        ...
        # save to image
        img = rgb * valid_mask + image * (1-valid_mask)
        return img
