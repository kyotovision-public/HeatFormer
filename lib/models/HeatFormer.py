...
from lib.utils.geometry import rot6d_to_rotmat, rot6d_to_rotmat_g, perspective_projection, matrix_to_axis_angle
        ...
        for iter in range(self.cfg.MODEL.ITERS + 1):
            if iter>0:
                ...
                if self.query_type == 1:
                    ...
                    q_x = rearrange(q_x, 'b v c h w -> (b v) c h w')
                ...
                # we use gt for training.
                # you can train using pseudo-gt(triangulation) instead of gt with HeatFormer_pseudo_gt.py instead of HeatFormer.py
                # Also, train without camera parameters by estimating weak-perspective camera parameter. (HeatFormer_trans_est.py)
                gt_3d = rearrange(target['c_3d'], 'b v j c -> (b v) j c')
                pred_3d = preds['regress_joints']

                # align for heatmap generation with ground-truth
                transl = gt_3d[:, 0] - pred_3d[:, 0]
